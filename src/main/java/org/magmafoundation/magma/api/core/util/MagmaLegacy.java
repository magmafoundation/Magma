 ...
 * lot more edge cases and invertible transformations (many of which are not immediately obvious)
 * than any other alternative. If you do make changes to this class please make sure to contribute
 * them back https://hub.spigotmc.org/stash/projects/SPIGOT/repos/craftbukkit/browse so that all may
 * benefit.
 *
 * @deprecated legacy use only
 */
@Deprecated
public class MagmaLegacy {

    private static final Map<Byte, Material> SPAWN_EGGS = new HashMap<>();
    private static final Set<String> whitelistedStates = new HashSet<>(
        Arrays.asList("explode", "check_decay", "decayable", "facing"));
    ...
    public static Item fromLegacyData(Material material, Item item, short data) {
        Preconditions.checkArgument(material.isLegacy(),
            "fromLegacyData on modern Material. Did you forget to define api-version: 1.13 in your plugin.yml?");
    ...
    public static Material valueOf(String name) {
        return (name.startsWith(Material.LEGACY_PREFIX)) ? Material.valueOf(name)
            : Material.valueOf(Material.LEGACY_PREFIX + name);
    }

    public static Material getMaterial(String name) {
        return (name.startsWith(Material.LEGACY_PREFIX)) ? Material.getMaterial(name)
            : Material.getMaterial(Material.LEGACY_PREFIX + name);
    }

    public static Material matchMaterial(String name) {
        return (name.startsWith(Material.LEGACY_PREFIX)) ? Material.matchMaterial(name)
            : Material.matchMaterial(Material.LEGACY_PREFIX + name);
    }
    ...
    static {
        SPAWN_EGGS
            .put((byte) 0, Material.PIG_SPAWN_EGG); // Will be fixed by updateMaterial if possible
        ...
        SPAWN_EGGS
            .put((byte) EntityType.ELDER_GUARDIAN.getTypeId(), Material.ELDER_GUARDIAN_SPAWN_EGG);
        ...
        SPAWN_EGGS
            .put((byte) EntityType.SKELETON_HORSE.getTypeId(), Material.SKELETON_HORSE_SPAWN_EGG);
        ...
        SPAWN_EGGS
            .put((byte) EntityType.TROPICAL_FISH.getTypeId(), Material.TROPICAL_FISH_SPAWN_EGG);
        ...
        SPAWN_EGGS
            .put((byte) EntityType.WITHER_SKELETON.getTypeId(), Material.WITHER_SKELETON_SPAWN_EGG);
        ...
        for (Material material : Material.values()) {
            ...
            if (material.isBlock()) {
                for (byte data = 0; data < 16; data++) {
                    ...
                    Dynamic blockTag = BlockStateFlatteningMap
                        .getFixedNBTForID(material.getId() << 4 | data);
                    ...
                    Block block = Registry.BLOCK.getValue(new ResourceLocation(name)).get();
                    if (block == null) {
                        continue;
                    }
                    ...
                    if (propMap.isPresent()) {
                        ...
                        for (String dataKey : properties.keySet()) {
                            ...
                            Preconditions.checkState(!properties.getString(dataKey).isEmpty(),
                                "Empty data string");
            ...
            for (byte data = 0; data < maxData; data++) {
                ...
                if (material
                    == Material.LEGACY_MONSTER_EGG /*&& data != 0 && EntityType.fromId(data) == null*/) { // Mojang broke 18w19b
                    continue;
                }
