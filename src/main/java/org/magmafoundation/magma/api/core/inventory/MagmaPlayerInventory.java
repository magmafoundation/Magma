...
/**
 * MagmaPlayerInventory
 *
 * @author Hexeption admin@hexeption.co.uk
 * @since 27/11/2019 - 05:46 am
 */
public class MagmaPlayerInventory extends MagmaInventory implements
    org.bukkit.inventory.PlayerInventory, EntityEquipment {

    public MagmaPlayerInventory(PlayerInventory inventory) {
        super(inventory);
    }
    ...
    public ItemStack getItemInMainHand() {
        return MagmaItemStack.asCraftMirror(getInventory().getCurrentItem());
    }
    ...
    public ItemStack getItemInOffHand() {
        return MagmaItemStack.asCraftMirror(getInventory().offHandInventory.get(0));
    }
    ...
    public void setItem(int index, ItemStack item) {
        ...
        ServerPlayerEntity player = ((MagmaPlayer) this.getHolder()).getHandle();
        ...
        player.connection
            .sendPacket(new SSetSlotPacket(player.container.windowId, index,
    ...
    public void setHeldItemSlot(int slot) {
        Validate.isTrue(slot >= 0 && slot < PlayerInventory.getHotbarSize(),
            "Slot is not between 0 and 8 inclusive");
        this.getInventory().currentItem = slot;
        ((MagmaPlayer) this.getHolder()).getHandle().connection
            .sendPacket(new SHeldItemChangePacket(slot));
    }
    ...
    public void setExtraContents(ItemStack[] items) {
        setSlots(items, getInventory().mainInventory.size() + getInventory().armorInventory.size(),
            getInventory().offHandInventory.size());
    }

    @Override
    public HumanEntity getHolder() {
        return (HumanEntity) ((IBridgeIInventory) inventory).getOwner();
    }
