...
public class ItemStackTest extends AbstractTestingBase {
    ...
    static abstract class StackProvider {
        ...
        /**
         * For each item in parameterList, it will apply nameFormat at nameIndex. For each item in parameterList for each item in materials, it will create a stack provider at each array index that
    ...
    static class CompoundOperator implements Operator {
         ...
         * @param singletonBitmask a list of bits representing the 'singletons' located in your originalLists. Lowest order bits represent the first items in originalLists. Singletons are
         * exponentially linked with each other, such that, the output will contain every unique subset of only items from the singletons, as well as every unique subset that contains at least one
         * item from each non-singleton.
         */
